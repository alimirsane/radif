...
export const PersonalUser = ({ closeModal }: { closeModal: () => void }) => {
  // get data
  ...
  const submit = (
    ...
    if (
      ...
      (values.role.includes("10" as unknown as never) ||
        values.role.includes("15" as unknown as never))
    ) {
      return;
    }
    let data = {
          ...
          <div className="grid grid-cols-1 gap-6 pb-3 text-right md:grid-cols-2">
            <div className="col-span-2 rounded-[8px] bg-secondary-light bg-opacity-30 p-4">
              ...
              <p className="pt-1 text-[14px]">
                این اطلاعات برای ثبت همکار ضروری می‌باشد.
              </p>
            </div>
            ...
            <div className={`col-span-2 md:col-span-1`}>
              <Select
                ...
                label={"سمت"}
                placeholder="سمت همکار راانتخاب کنید"
              ...
              {formik.values?.role?.length === 1 &&
                (formik.values.role?.includes("10" as unknown as never) ||
                  formik.values.role?.includes("15" as unknown as never)) && (
