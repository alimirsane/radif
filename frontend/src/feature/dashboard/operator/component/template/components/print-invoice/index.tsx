...
export const InvoicePrint = (props: RequestPrintProps) => {
  const { request, invoiceType } = props;
  const laboratory = useMemo(() => {
    return request?.experiment_obj?.laboratory_obj;
  }, [request]);
          ...
          (child?.parameter_obj?.reduce((paramSum, parameter) => {
            ...
            const quantity =
              ...
              child?.experiment_obj?.test_unit_type === "نمونه"
                ...
                : child?.test_duration ?? 1;

            return paramSum + unitPrice * quantity;
          }, 0) ?? 0)
        );
      }, 0);
  }, [request, getSamplesFlattenedForms]);
  ...
  return (
    <div className="no-padding hidden w-full text-[9px]" id="print-transaction">
        ...
        <div className="flex justify-end">
          <div className="w-[80%] border-[1px]">
            <div className="grid grid-cols-2 text-center">
              ...
              <span className="py-1">{request?.request_number}</span>
            </div>
        ...
        <div className="w-[55%]">
          <p className="mb-[6px]">
            ...
            <span>
              ...
              {request?.owner_obj?.first_name +
                " " +
                request?.owner_obj?.last_name}
            </span>
          </p>
          <p className="mb-[6px]">
            <span>نشانی: </span>
        ...
        <div className="w-[22.5%]">
          <p className="mb-[6px]">
            ...
            <span>
              ...
              {/* ****** MUST BE CHANGED AFTER ADDING PAYER_OBJ ****** */}
              {request?.owner_obj?.account_type === AccountType.BUSINESS
                ? request?.owner_obj?.company_telephone
                : request?.owner_obj?.telephone ?? "---"}
            </span>
          </p>
          <p className="mb-[6px]">
            ...
            <span>
              {/* ****** MUST BE CHANGED AFTER ADDING PAYER_OBJ ****** */}
              {/* {"0" + request?.payer_obj?.username?.slice(3)} */}
              {/* {"0" + request?.owner_obj?.username?.slice(3)} */}
              {request?.owner_obj?.account_type === AccountType.PERSONAL
                ? request?.owner_obj?.username
                  ? `0${request?.owner_obj?.username.slice(3)}`
                  : ""
                : !!request?.owner_obj?.linked_users_objs?.length &&
                    request?.owner_obj?.linked_users_objs[0]?.username
                  ? `0${request?.owner_obj?.linked_users_objs[0]?.username.slice(3)}`
                  : ""}
            </span>
          </p>
          {request?.owner_obj?.account_type === AccountType.BUSINESS && (
            <p className="mb-[6px]">
              <span>نام سازمان: </span>
              <span>{request?.owner_obj?.company_name ?? "-"}</span>
            </p>
          )}
          {/* <p className="mb-[12px]">
            <span>دورنگار: </span>
        ...
        <div className="w-[22.5%]">
          <p className="mb-[6px]">
            ...
            <span>
              ...
              {request?.owner_obj?.account_type === AccountType.BUSINESS
                ? request?.owner_obj?.company_national_id
                : request?.owner_obj?.national_id}
            </span>
          </p>
          <p className="mb-[6px]">
            <span>ایمیل: </span>
          ...
          {request?.owner_obj?.account_type === AccountType.BUSINESS && (
            <p className="mb-[6px]">
              <span>شماره اقتصادی: </span>
              <span>{request?.owner_obj?.company_economic_number ?? "-"}</span>
            </p>
          )}
        </div>
      </div>
      ...
      <div className="flex border-[1px]">
        ...
        <div className="text-cenetr flex w-[6%] items-center justify-center border-l-[1px] py-[8px]">
          {/* {(
            (Number(request?.price_wod) * Number(request?.discount ?? 0)) /
            100
          ).toLocaleString()} */}
        ...
        <div className="flex w-[74%]">
          <div className="back-gray flex w-[42%] flex-col items-center justify-center gap-1 border-l-[1px] text-center">
            ...
            <span>
              ...
              {Number(request?.labsnet_discount).toLocaleString()} (ریال)
            </span>
            {/* {request?.labsnet_code1 && <span>1. {request?.labsnet_code1}</span>}
            {request?.labsnet_code2 && <span>2. {request?.labsnet_code2}</span>} */}
            <span>{request?.labsnet_description}</span>
          </div>
          <div className="flex w-[42%] flex-col items-center justify-center gap-1 border-l-[1px] text-center">
            {/* مجموع هزینه خدمات اعتباری:{" "} */}
            <span>
              مجموع گرنت پژوهشی:{" "}
              {Number(request?.grant_request_discount).toLocaleString()} (ریال)
            </span>
            {/* {request?.grant_request1 && (
              <span>
                1.{" گرنت اعطایی "}
                {request?.grant_request1_obj?.receiver_obj.first_name}{" "}
                {request?.grant_request1_obj?.receiver_obj.last_name}
              </span>
            )}
            {request?.grant_request2 && (
              <span>
                2.{" گرنت اعطایی "}
                {request?.grant_request2_obj?.receiver_obj.first_name}{" "}
                {request?.grant_request2_obj?.receiver_obj.last_name}
              </span>
            )} */}
          </div>
          <div className="back-gray flex w-[16%] items-center justify-center border-l-[1px] text-center">
            {(
              Number(request?.labsnet_discount) +
              Number(request?.grant_request_discount)
            ).toLocaleString()}{" "}
            (ریال)
          </div>
        </div>
          ...
          request?.latest_status_obj?.step_obj?.name === "در انتظار پرداخت" ? (
            ...
            <>
              <p className="mb-[8px]">
                <span>تاریخ پرداخت: </span>
                <span>
                  {DateHandler.formatDate(request?.order_obj?.[0]?.updated_at, {
                    year: "numeric",
                    month: "2-digit",
                    day: "2-digit",
                  })}
          ...
          <p className="mb-[16px]">
            ...
            <span>
              {/* {request?.result_objs && request?.result_objs[0]?.result_by_obj
                ...
                : ""} */}
              {!!request?.status_objs?.length
                ? request?.status_objs[1]?.action_by_name
                : ""}
            </span>
          </p>
        </div>
