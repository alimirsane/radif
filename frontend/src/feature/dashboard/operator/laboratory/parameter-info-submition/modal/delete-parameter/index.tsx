...
const DeleteParameter = () => {
  ...
  // get modal data
  const parameter = useModalHandler((state) => state.modalData);
  // delete parameter api
  const { mutateAsync } = useMutation(
    apiParameter(true, {
      success: "حذف پارامتر موفقیت آمیز بود",
      fail: "حذف پارامتر انجام نشد",
      waiting: "در حال انتظار",
    }).delete(parameter.id),
  );
  // delete parameter
  const deletedParameterHandler = () => {
    mutateAsync(parameter.id)
      .then((res) => {
        // refetch data
  ...
  return (
    <Card color={"white"} className="w-[90vw] p-6 md:w-[40vw] lg:w-[30vw]">
      <span className="mb-[16px] flex flex-row items-center justify-between">
        <h6 className="text-[20px] font-[700]">حذف پارامتر</h6>
